...
public class EsOperatorImplTest {
    ...
    public static final String DEFAULT_MAPPINGS = "{\"dynamic\":true,\"_source\":{\"enabled\":true},\"_all\":{\"enabled\":false},\"properties\":{\"user\":{\"type\":\"keyword\"},\"age\":{\"type\":\"integer\"},\"content\":{\"type\":\"text\",\"analyzer\":\"ik_smart\"},\"date\":{\"type\":\"date\",\"format\":\"yyyy-MM-dd HH:mm:ss||yyyy-MM-dd HH:mm:ss.s||yyyy-MM-dd||epoch_millis\"}}}";
    ...
    class TestGetOrMultiGet {
        ...
        public void before() {
            ...
            BulkRequestBuilder deleteBulkBuilder = esOperator.getClient().prepareBulk();
            for (int i = 0; i < 2; i++) {
                DeleteRequestBuilder deleteRequestBuilder = esOperator.getClient().prepareDelete(INDEX_NAME, EsOperatorImpl.DEFAULT_TYPE, String.format("grd-%d", i));
                deleteBulkBuilder.add(deleteRequestBuilder);
            }
            deleteBulkBuilder.get();
            BulkRequestBuilder createBulkBuilder = esOperator.getClient().prepareBulk();
            for (int i = 0; i < 2; i++) {
                TestPojo testPojo = new TestPojo();
                testPojo.setUser(String.format("grd-%d", i));
                IndexRequestBuilder indexRequestBuilder = esOperator.getClient()
                        .prepareIndex(INDEX_NAME, EsOperatorImpl.DEFAULT_TYPE, testPojo.getUser())
                        .setSource(JSONObject.toJSONString(testPojo), XContentType.JSON);
                createBulkBuilder.add(indexRequestBuilder);
            }
            createBulkBuilder.get();
